    ...
    return (
        <div className="quotation-container">
            ...
            <div className="a4-paper" ref={printRef}>
                {loading ? (
                    <div style={{ padding: '40px', textAlign: 'center' }}>กำลังโหลดข้อมูลใบเสนอราคา...</div>
                ) : !quote ? (
                    <div style={{ padding: '40px', textAlign: 'center', color: 'red' }}>ไม่พบข้อมูลใบเสนอราคา หรือคุณยังไม่ได้ระบุ ID</div>
                ) : (
                <>
                ...
                <div className="customer-info-grid">
                    ...
                    <div className="info-box" style={{ gridColumn: '2 / 3' }}>
                        ...
                        <span className="info-value">{new Date(quote.quote_date || quote.created_at).toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                    </div>
                        ...
                        {quote.items?.map((item, idx) => (
                            <tr key={idx}>
                                ...
                                <td className="col-desc">
                                    <div className="item-main">{item.desc}</div>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2px 8px' }}>
                                        {item.sub_desc && <span className="item-sub">- {item.sub_desc}</span>}
                                        {(item.item_width || item.item_height) && <span className="item-sub">ขนาด: กว้าง {item.item_width || '-'} ม. x สูง {item.item_height || '-'} ม.</span>}
                                        {item.item_style && <span className="item-sub">รูปแบบ: {item.item_style}</span>}
                                        {item.item_spec && <span className="item-sub">สเปค: {item.item_spec}</span>}
                                    </div>
                                </td>
                                <td className="col-qty">{item.qty}</td>
                                <td className="col-price">{formatMoney(item.price)}</td>
                                <td className="col-amount">{formatMoney(Number(item.qty) * Number(item.price))}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <div className="summary-container">
                    <div className="amount-in-words">
                        ({ThaiBahtText(quote.total_amount)})
                    </div>
                    <div className="summary-totals">
                        <div className="summary-row">
                            <div className="summary-label">รวมเป็นเงิน</div>
                            <div className="summary-value">{formatMoney(quote.sub_total)}</div>
                        </div>
                        {quote.vat_rate > 0 && (
                            <div className="summary-row">
                                <div className="summary-label">ภาษีมูลค่าเพิ่ม {quote.vat_rate}%</div>
                                <div className="summary-value">{formatMoney(quote.vat_amount)}</div>
                            </div>
                        )}
                        {quote.wht_rate > 0 && (
                            <div className="summary-row" style={{ color: 'var(--text-muted)', fontStyle: 'italic', fontSize: '13px' }}>
                                <div className="summary-label">*(หมายเหตุ: ยอดนี้ยังไม่รวมการหัก ณ ที่จ่าย {quote.wht_rate}%)</div>
                                <div className="summary-value"></div>
                            </div>
                        )}
                        <div className="summary-row">
                            <div className="summary-label" style={{ fontWeight: 700 }}>จำนวนเงินทั้งสิ้น</div>
                            <div className="summary-value" style={{ fontWeight: 700 }}>{formatMoney(quote.total_amount)}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="signatures">
                    <div className="signature-box">
                        ...
                        <div style={{ fontWeight: 700 }}>ผู้เสนอราคา</div>
                        <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>วันที่ ........./........./.........</div>
                    </div>
                    <div className="signature-box">
                        <div className="signature-line"></div>
                        <div style={{ fontWeight: 700 }}>ผู้อนุมัติสั่งซื้อ</div>
                        <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>วันที่ ........./........./.........</div>
                    </div>
                </div>
                </>
